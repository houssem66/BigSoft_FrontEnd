...
function Index() {
  ...
  useEffect(() => {
      if (Fetch) {
          setFetch(false)
          let params = { include: "Client.Grossiste,DetailsDevis.Produit" }
  ...
  const handleEdit = (item) => {
      ...
      navig('/feed/devis_edit/', { state: { Bon: item } });

  };
  const handleDetails = (item) => {

      navig('/feed/devis_details/', { state: { Bon: item } });
...
>
    <Grid sx={{ mt: 2, ml: 5, mx: 2 }} container spacing={5}>
        ...
        <Grid item md={2}>
            <Button css={{ width: "100%" }} flat color="success" onClick={event => { navig("/feed/devis_ajout"); }} auto icon={<AddIcon />}>Ajouter</Button>
