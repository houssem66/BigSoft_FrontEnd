...
function Index() {
  ...
  useEffect(() => {
      if (Fetch) {
          setFetch(false)
          let params = { include: "Client.Grossiste,DetailsCommandes.Produit" }
  ...
  const handleEdit = (item) => {
      ...
      navig('/feed/bonCommandeClient_edit/', { state: { Bon: item } });

  };
  const handleDetails = (item) => {

      navig('/feed/bonCommandeClient_details/', { state: { Bon: item } });
...
>
    <Grid sx={{ mt: 2, ml: 5, mx: 2 }} container spacing={5}>
        ...
        <Grid item md={2}>
            <Button css={{ width: "100%" }} flat color="success" onClick={event => { navig("/feed/bonCommandeClient_ajout"); }} auto icon={<AddIcon />}>Ajouter</Button>
